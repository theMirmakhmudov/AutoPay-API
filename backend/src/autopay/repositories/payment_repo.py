...
class PaymentRepository:
    ...
    def create_intent(
        self,
        merchant_id: str,
        base_amount_tiyins: int,
        expected_amount_tiyins: int,
        expires_at: datetime,
    ) -> PaymentIntent:
    ...
    def get_active_intent_by_amount(
        self, merchant_id: str, amount_tiyins: int
    ...
    def get_max_expected_amount_for_base(
        self, merchant_id: str, base_amount_tiyins: int
    ) -> Optional[int]:
        """
    ...
    def expire_old_intents(self) -> int:
        ...
        expired = (
            self.db.query(PaymentIntent)
            .filter(PaymentIntent.status == "PENDING", PaymentIntent.expires_at <= now)
            .all()
        )
    ...
    def payment_exists(self, message_id: int, chat_username: str, merchant_id: str) -> bool:
        ...
        exists_processed = (
            self.db.query(ProcessedPayment)
            .filter(
