...
class CreatePaymentRequest(BaseModel):
    base_amount: float = Field(
        ..., description="Amount in UZS", json_schema_extra={"example": 35000}
    )
...
class CreatePaymentResponse(BaseModel):
    ...
    has_collision: bool = Field(
        ..., description="True if someone else is paying the same amount right now"
    )
    force_wait: bool = Field(
        False, description="True if dynamic offset > 10 UZS — tell user to wait"
    )
