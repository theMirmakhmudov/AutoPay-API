...
EMOJIS = {
    "stop": "5395695537687123235",
    "sad": "5366116089329646883",
    "warning": "5420323339723881652",
    "halt": "5283283384418707920",
    "cross": "5800887979366944343",
    "check": "5427009714745517609",
    "party": "5461151367559121950",
    "rocket": "5363945077850799577",
    "money": "5350452584119279096",
    "bank": "5264895611517300926",
    "stats": "5431577498364158238",
    "chart": "5431736674147114227",
    "lock": "5472308992514464048",
    "key": "5330115548900501467",
    "shield": "5251203410396458957",
    "id": "5422683699130933153",
    "phone": "5465169893580086142",
    "numbers": "5377624166436445368",
    "green_circle": "5981066684977384749",
    "red_circle": "5981335554225081686",
    "trash": "5775903905498010383",
    "globe": "5999317873623831250",
    "email_send": "5406631276042002796",
    "email_recv": "5253742260054409879",
    "memo": "5373251851074415873",
    "megaphone": "5424818078833715060",
    "group": "5372926953978341366",
...
def get_admin_keyboard():
    ...
    builder.row(
        InlineKeyboardButton(
            text="Stats", callback_data="admin_stats", icon_custom_emoji_id=EMOJIS["stats"]
        ),
        InlineKeyboardButton(
            text="Merchants", callback_data="admin_merchants", icon_custom_emoji_id=EMOJIS["group"]
        ),
    )
    builder.row(
        InlineKeyboardButton(
            text="Revenue", callback_data="admin_revenue", icon_custom_emoji_id=EMOJIS["money"]
        ),
        InlineKeyboardButton(
            text="Recent", callback_data="admin_recent", icon_custom_emoji_id=EMOJIS["chart"]
        ),
    )
    builder.row(
        InlineKeyboardButton(
            text="Errors", callback_data="admin_errors", icon_custom_emoji_id=EMOJIS["warning"]
        ),
        InlineKeyboardButton(
            text="Broadcast",
            callback_data="admin_broadcast",
            icon_custom_emoji_id=EMOJIS["megaphone"],
        ),
    )
    builder.row(
        InlineKeyboardButton(
            text="View All Payments",
            callback_data="admin_payments_0",
            icon_custom_emoji_id=EMOJIS["stats"],
        )
    )
    builder.row(
        InlineKeyboardButton(
            text="Add Merchant",
            callback_data="admin_add_merchant",
            icon_custom_emoji_id=EMOJIS["plus"],
        ),
        InlineKeyboardButton(
            text="Close", callback_data="admin_close", icon_custom_emoji_id=EMOJIS["cross"]
        ),
    )
        ...
        for aid in admin_ids:
            try:
                await bot.set_my_commands(
                    admin_cmds + user_cmds, scope=BotCommandScopeChat(chat_id=aid)
                )
    ...
    if not is_admin(message.from_user.id):  # type: ignore
        await message.answer_rich(
            rich_message=InputRichMessage(
                markdown=f"{e('stop', '⛔')} **Kechirasiz!** Ushbu amallarni bajarish uchun huquqingiz yo'q."
            )
        )
        return
...
"""
    ...
    if isinstance(target, CallbackQuery):
        ...
        await target.answer_rich(rich_message=InputRichMessage(markdown=text), reply_markup=markup)
...
async def send_merchants(target: (Message | CallbackQuery)):
    ...
    blocks = [RichBlockSectionHeading(size=1, text=[emoji("group", "👥"), " Registered Merchants"])]
    ...
    if not merchants:
        ...
        for m in merchants:
            ...
            cells.append(
                [
                    RichBlockTableCell(
                        align="left", valign="middle", text=[emoji("person", "🧑‍💼"), f" {phone}"]
                    ),
        ...
        blocks.append(
            RichBlockDetails(
                ...
                blocks=[RichBlockTable(is_bordered=True, is_striped=True, cells=cells)],
            )
        )
    ...
    if isinstance(target, CallbackQuery):
        await target.message.edit_text(rich_message=msg, reply_markup=markup)  # type: ignore
    else:
...
async def cmd_ban(message: Message):
    if not is_admin(message.from_user.id):  # type: ignore
        return
    parts = message.text.split(" ", 1)  # type: ignore
    if len(parts) < 2:
        await message.answer_rich(
            rich_message=InputRichMessage(markdown="Please provide merchant ID.")
        )
        return
    ...
    if not merchant:
        ...
        await message.answer_rich(
            rich_message=InputRichMessage(
                markdown=f"{e('sad', '😔')} **Afsuski!** Sizning profilingiz tizimdan topilmadi."
            )
        )
        return
    ...
    await message.answer_rich(
        rich_message=InputRichMessage(
            markdown=f"{e('check', '✅')} **Ajoyib!** Merchant `{merchant_id}` muvaffaqiyatli ban qilindi va tizimdan uzildi."
        )
    )
...
async def cmd_start(message: Message, state: FSMContext):
    await _cleanup_state(message.from_user.id, state)  # type: ignore
    if is_admin(message.from_user.id):  # type: ignore
        await message.answer_rich(
            rich_message=InputRichMessage(markdown=f"**{e('shield', '🛡️')} Admin Control Panel**"),
        ...
        if not is_allowed:
            await message.answer_rich(
                rich_message=InputRichMessage(
                    markdown=f"""{e("cross", "❌")} **Access Denied.**
...
You are not an authorized merchant. Please contact the administrator."""
        ...
        await message.answer_rich(
            rich_message=InputRichMessage(
                markdown="""**👋 Welcome to Auto Payment Gateway!**
...
async def cmd_credentials(message: Message):
    db = SessionLocal()
    merchant_name = f"Merchant_{message.from_user.id}"  # type: ignore
    merchant = db.query(Merchant).filter(Merchant.name == merchant_name).first()
    db.close()
    if not merchant:
        await message.answer_rich(
            rich_message=InputRichMessage(
                markdown=f"""{e("warning", "⚠️")} **Tizimga ulanmagansiz!**
Iltimos, /start orqali ulaning."""
    ...
    await message.answer_rich(
        rich_message=InputRichMessage(
            markdown=f"""**{e("lock", "🔐")} Your Credentials**
...
async def cmd_setcard(message: Message):
    parts = message.text.split(" ", 1)  # type: ignore
    if len(parts) < 2:
        await message.answer_rich(rich_message=InputRichMessage(markdown="Usage: /setcard *4183"))
        return
    mask = parts[1].strip()
    ...
    if not mask_digits.isdigit() or len(mask_digits) < 4:
        await message.answer_rich(
            rich_message=InputRichMessage(
                markdown=f"""{e("warning", "⚠️")} **Noto'g'ri format!**
Iltimos, kartaning oxirgi 4 raqamini kiriting."""
    ...
    if merchant:
        ...
        await message.answer_rich(
            rich_message=InputRichMessage(
                markdown=f"""💳 **Muvaffaqiyatli saqlandi!**
Karta filtri: `{mask}` etib belgilandi."""
        ...
        await message.answer_rich(
            rich_message=InputRichMessage(
                markdown=f"{e('sad', '😔')} **Afsuski!** Sizning profilingiz topilmadi."
            )
        )
    db.close()
...
async def cmd_unsetcard(message: Message):
    if is_admin(message.from_user.id):  # type: ignore
        return
    db = SessionLocal()
    merchant = db.query(Merchant).filter(Merchant.telegram_id == str(message.from_user.id)).first()  # type: ignore
    if not merchant:
        db.close()
        await message.answer_rich(
            rich_message=InputRichMessage(
                markdown=f"{e('sad', '😔')} **Siz tizimdan ro'yxatdan o'tmagansiz.**"
            )
        )
        return
    ...
    db.close()
    await message.answer_rich(
        rich_message=InputRichMessage(markdown=f"{e('trash', '🗑️')} **Karta filtri o'chirildi!**")
    )
...
async def cmd_setwebhook(message: Message):
    ...
    if len(parts) < 2:
        await message.answer_rich(
            rich_message=InputRichMessage(markdown="Usage: /setwebhook https://...")
        )
        return
    url = parts[1].strip()
    if not url.startswith("http://") and not url.startswith("https://"):
        await message.answer_rich(
            rich_message=InputRichMessage(
                markdown=f"{e('globe', '🌐')} **Noto'g'ri Webhook manzili!**"
            )
        )
        return
    db = SessionLocal()
    ...
    if not merchant:
        db.close()
        await message.answer_rich(
            rich_message=InputRichMessage(markdown=f"{e('sad', '😔')} **Tizimga ulanmagansiz.**")
        )
        return
    ...
    await message.answer_rich(
        rich_message=InputRichMessage(
            markdown=f"""{e("rocket", "🚀")} **Webhook saqlandi:**
`{url}`"""
        )
    )
...
Bot endi SMS xabarlaringizni o'qimaydi."""
        ...
        await message.answer_rich(
            rich_message=InputRichMessage(
                markdown=f"{e('warning', '⚠️')} **Siz tizimga ulanmagansiz!**"
            )
        )
    db.close()
...
async def process_phone(message: Message, state: FSMContext):
    ...
    if not text.startswith("+") or len(text) < 10:
        await message.answer_rich(
            rich_message=InputRichMessage(
                markdown=f"""{e("phone", "📱")} **Telefon raqam noto'g'ri!**
Format: `+998901234567`"""
            )
        )
        return
    await message.answer_rich(
        rich_message=InputRichMessage(
            markdown=f"{e('email_send', '📨')} **Tasdiqlash kodi yuborilmoqda...**"
        )
    )
    ...
    try:
        ...
        user_states[message.from_user.id] = {"temp_client": temp_client}  # type: ignore
        await state.update_data(phone=text, phone_code_hash=sent.phone_code_hash)
        await state.set_state(AuthFlow.AWAITING_CODE)
        await message.answer_rich(
            rich_message=InputRichMessage(
                markdown=f"""{e("email_recv", "📩")} Code sent! Reply with:
`CODE 12345`"""
            )
        )
    except Exception as ex:
        ...
        await message.answer_rich(
            rich_message=InputRichMessage(markdown=f"{e('cross', '❌')} Failed: {ex}")
        )
        await _cleanup_state(message.from_user.id, state)  # type: ignore
...
async def process_code(message: Message, state: FSMContext):
    text = message.text.strip().upper()  # type: ignore
    if not text.startswith("CODE "):
        await message.answer_rich(
            rich_message=InputRichMessage(
                markdown=f"""{e("warning", "⚠️")} **Format:**
`CODE 12345`"""
            )
        )
        return
    ...
    udata = user_states.get(message.from_user.id, {})  # type: ignore
    temp_client: TelegramClient = udata.get("temp_client")
    if not temp_client:
        await message.answer_rich(
            rich_message=InputRichMessage(
                markdown=f"{e('cross', '❌')} Xatolik: Sessiya topilmadi. /start"
            )
        )
        await state.clear()
        return
    try:
        ...
        await message.answer_rich(
            rich_message=InputRichMessage(
                markdown=f"{e('lock', '🔐')} Ikki bosqichli parol o'rnatilgan. Parolni yuboring:"
            )
        )
    except Exception as ex:
        ...
        await message.answer_rich(
            rich_message=InputRichMessage(
                markdown=f"{e('cross', '❌')} **Ulanishda xatolik:** {ex}"
            )
        )
        await _cleanup_state(message.from_user.id, state)  # type: ignore
...
async def process_2fa(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer_rich(
            rich_message=InputRichMessage(markdown=f"{e('cross', '❌')} **Xato:** {ex}")
        )
        await _cleanup_state(message.from_user.id, state)  # type: ignore


async def _finish_login(
    message: Message, temp_client: TelegramClient, phone: str, state: FSMContext
):
    ...
    await state.clear()
    user_states.pop(message.from_user.id, None)  # type: ignore
    if _client_manager:
    ...
    if is_new:
        ...
        msg += f"{e('warning', '⚠️')} *API Kalitni hoziroq saqlab oling. Boshqa ko'rsatilmaydi!*\n"
...
async def process_broadcast(message: Message, state: FSMContext):
    ...
    if not text:
        await message.answer_rich(
            rich_message=InputRichMessage(markdown=f"{e('memo', '📝')} **Xabar bo'sh!**")
        )
        return
    await message.answer_rich(
        rich_message=InputRichMessage(
            markdown=f"{e('rocket', '🚀')} **Barcha merchantlarga yuborilmoqda...**"
        )
    )
    db = SessionLocal()
    merchants = db.query(Merchant).all()
...
{text}""",
    ...
    await message.answer_rich(
        rich_message=InputRichMessage(
            markdown=f"{e('check', '✅')} **E'lon yuborildi!** Jami {success} ta."
        )
    )
...
async def cb_payments(query: CallbackQuery):
    if not is_admin(query.from_user.id):
        return
    page = int(query.data.split("_")[2])  # type: ignore
    per_page = 5
...
"""
    ...
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton(text="Prev", callback_data=f"admin_payments_{page - 1}"))
    if offset + per_page < total_count:
        nav.append(InlineKeyboardButton(text="Next", callback_data=f"admin_payments_{page + 1}"))
    if nav:
        builder.row(*nav)
...
async def process_new_merchant(message: Message, state: FSMContext):
    ...
    if not text.isdigit():
        await message.answer_rich(
            rich_message=InputRichMessage(
                markdown=f"{e('numbers', '🔢')} **Noto'g'ri Telegram ID!**"
            )
        )
        return
    db = SessionLocal()
    ...
    if existing:
        db.close()
        await message.answer_rich(
            rich_message=InputRichMessage(markdown=f"{e('check', '✅')} **Oldin qo'shilgan!**")
        )
        await state.clear()
        return
    ...
    await message.answer_rich(
        rich_message=InputRichMessage(
            markdown=f"{e('party', '🎉')} **Merchant qo'shildi!** `{text}` ulanishi mumkin."
        )
    )
